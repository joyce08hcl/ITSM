...
def raise_ticket_format(user, sap_id, query, response):
...
# Streamlit app
def main():
    st.title("User Query Assistance")

    # Input fields
    user_name = st.text_input("Enter your name:")
    user_id = st.text_input("Enter your SAP ID:")
    ...
    if st.button("Submit Query"):
        ...
        if router.get("department") == "IT Admin":
            ...
            # After extracting ticket details
            ticket_details_list = []
            for doc in docs:
                ticket_details = extract_ticket_details(doc)
                st.success(ticket_details)
